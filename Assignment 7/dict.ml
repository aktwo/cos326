...
struct
  ...
  let test_insert_in_order () =
    ...
    List.iter (fun (k,v) -> assert(lookup d2 k = Some v)) pairs2 ;
    assert(balanced d2) ;
    ()
  ...
  let test_insert_empty () =
    ...
    List.iter (fun (k,v) -> assert(lookup d3 k = Some v)) pairs3 ;
    assert(balanced d3) ;
    ()
  ...
  let test_insert_reversed_order () =
    ...
    List.iter (fun (k,v) -> assert(lookup d6 k = Some v)) pairs6 ;
    assert(balanced d6) ;

    let pairs7 = generate_pair_list 13 in
    let d7 = insert_list_reversed empty pairs7 in
    List.iter (fun (k,v) -> assert(lookup d7 k = Some v)) pairs7 ;
    assert(balanced d7) ;
    ()
  ...
  let test_insert_random_order () =
    ...
    List.iter (fun (k,v) -> assert(member d5 k)) pairs5 ;
    assert(balanced d5) ;
    ()
