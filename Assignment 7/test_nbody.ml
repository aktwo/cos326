...
let lineup_bodies =
  let m = testM in
  let v = origin in
  Seq.tabulate(fun x->(m, (1.0+. 15.*.(float_of_int x),5.0),v)) 10
let lineup_theta =
  std_theta
...
let diamond_bodies =
  ...
  let pos = function
   ...
   | 6 -> ((50.),(0.))
   | _ -> ((25.),(25.)) in
  Seq.tabulate (fun n ->(m,pos n,v)) 8
...
let twister_bodies =
  ...
  Seq.tabulate(fun n->(m, (pos n), speed n)) numBodies
...
let orbit_bodies =
  ...
  Seq.cons
    (bigM,(xI,yI),v)
    (Seq.tabulate (fun n ->(lilM, pos n, speed n)) 4)
