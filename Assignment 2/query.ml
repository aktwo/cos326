...
let by_studio (movies:movie list) : studio_gross list =
  (* helper method to add a movie to the studio list *)
  ...
  in
  (* helper method to sequentially process movies (i.e. add them 
     to the studio list) *)
...
assert(take 1 (decade 0 data4) = data1);;
assert((drop 1 data2) = (drop 1 (take 2 data4)));;
assert((drop 2 (sort_by_year data3)) = (drop 2 data3));;

(* addt'l testing (commented out for grading scripts) 
   decade 0 data4;;
   average data4;;
   sort_by_gross data4;;
   sort_by_year data4;;
   sort_by_studio (by_studio data4);;
*)
