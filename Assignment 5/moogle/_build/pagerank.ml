...
struct
  ...
  exception ScrewedUpGraphException

  let rank graph = 
    let node_map = NS.zero_node_score_map (G.nodes graph) in
    let get_rand_neighbor node_list = 
      List.nth node_list (Random.int (List.length node_list))
    in
    let rec traverse_graph_random num_steps = 
      match G.get_random_node graph with
      | None -> node_map
      | Some node -> (
	if (num_steps <= 0) then NS.add_score node_map node 1.0
	else NS.add_score (traverse_graph_neighbor node (num_steps-1)) node 1.0
      )
    and traverse_graph_neighbor node num_steps = 
      if (num_steps <= 0) then NS.add_score node_map node 1.0
      else ( 
	match G.neighbors graph node with 
	| None -> raise ScrewedUpGraphException
	| Some node_list -> (
	  match P.do_random_jumps with 
	  | None -> (
	    match node_list with
	    | [] -> NS.add_score (traverse_graph_random (num_steps-1)) node 1.0
	    | ls ->
	      NS.add_score (traverse_graph_neighbor (get_rand_neighbor ls) (num_steps-1)) node 1.0 
	  )
	  | Some prob -> (
    ...
    in
    NS.normalize (traverse_graph_random P.num_steps)
end
...
module TestRandomWalkRankerLoop =
struct 
  module G = NamedGraph
  let g = G.from_edges [("a","b") ; 
                        ("b","c") ;
...
(*  let _ = 
    Printf.printf 
      ("Testing RandomWalkRanker with loop (should be equal probs):\n NS: %s\n") 
      (NS.string_of_node_score_map ns) 
*)    
