...
struct
  module D = Dict.Make(
    struct
      ...
      let gen_value (u:unit) = ()
      let gen_pair (u:unit) = (gen_key () , gen_value())
    end
  )
  ...
  let is_empty set = 
    ...
    | None -> true
    | Some _ -> false
  let insert element set = 
    match D.lookup set element with
    | None -> D.insert set element ()
    | Some _ -> set
  let singleton element = insert element empty
  let remove element set = D.remove set element
  let member set element = D.member set element
  let choose set = 
    match D.choose set with
    | None -> None 
    | Some (key, _, dict) -> Some (key, dict)
  let fold f a set = D.fold (fun key value last -> f key last) a set
  let union set1 set2 = fold insert set1 set2
  let intersect set1 set2 =
    let create_intersect key base_set =
      if (member set2 key) then insert key base_set else base_set 
    in
    fold create_intersect empty set1
  ...
  let rec generate_random_list (size: int) : D.key list = 
    match size with
    | 0 -> []
    | n -> (C.gen_random ()) :: generate_random_list (n-1)

  let rec generate_random_dict (size: int) : D.dict =
    if size <= 0 then empty
    else (
      let the_keys = generate_random_list size in
      List.fold_left (fun dict key -> if (D.member dict key) then dict else (D.insert dict key ())) D.empty the_keys
    )
      
  let test_insert () =
    let rand_dict = generate_random_dict 100 in
    let inserted_dict = D.fold (fun key value base -> insert key base) D.empty rand_dict in
    D.fold (fun key value base -> assert(D.member rand_dict key)) () inserted_dict ;
    ()

  let test_remove () =
    let rand_dict = generate_random_dict 100 in
    let deleted_dict = D.fold (fun key value base -> remove key base) rand_dict rand_dict in
    D.fold (fun key value base -> assert(not(D.member rand_dict key))) () deleted_dict ;
    ()

  let test_is_empty () =
    let empty_dict = D.empty in
    assert(is_empty empty_dict); 
    ()

  let test_singleton () = 
    let key = C.gen_random() in
    let one_item_dict = singleton key in
    assert (
      match D.choose one_item_dict with
      | None -> false 
      | Some (ret_key, ret_val, ret_dict) -> if ((ret_key = key) && (is_empty ret_dict)) then true else false
    );
    ()

  let test_union () =
    let rand_dict1 = generate_random_dict 100 in
    let rand_dict2 = generate_random_dict 100 in
    let union_dict = union rand_dict1 rand_dict2 in
    D.fold (fun key value base -> assert((D.member rand_dict1 key) || (D.member rand_dict2 key))) () union_dict;
    ()

  let test_intersect () =
    let rand_dict1 = generate_random_dict 100 in
    let rand_dict2 = generate_random_dict 100 in
    let int_dict = intersect rand_dict1 rand_dict2 in
    D.fold (fun key value base -> 
      assert(D.member rand_dict1 key); 
      assert(D.member rand_dict2 key);) () int_dict;
    ()

  let test_member () =
    let key_in_dict = C.gen_random () in
    let key_not_in_dict = C.gen_random () in
    let dict = singleton key_in_dict in
    assert((member dict key_in_dict) && not(member dict key_not_in_dict));
    ()
  ...
  let test_choose () =
    ...
    let one_item_dict = singleton key in
    assert(
      match choose one_item_dict with
      | None -> false
      | Some (ret_key, ret_dict) -> (ret_key=key) && (is_empty ret_dict)
    );
    let no_item_dict = empty in
    assert(
      match choose no_item_dict with
      | None -> true
      | Some _ -> false
    );
    ()
  ...
  let test_fold () =
    let rand_dict = generate_random_dict 100 in
    fold (fun element base -> assert (member rand_dict element)) () rand_dict;
    ()
