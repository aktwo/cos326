...
struct
  ...
  exception TODO
  exception DuplicateKeyException of string
  ...
  let rec fold (f: key -> value -> 'a -> 'a) (u: 'a) (d: dict) : 'a =
    match d with 
    | Leaf -> u
    | Two (left, (key, value), right) -> fold f (fold f (f key value u) left) right
    | Three (left, (key1, value1), mid, (key2, value2), right) ->
      fold f (fold f (fold f (f key1 value1 (f key2 value2 u)) left) mid) right
  ...
  let insert_upward_two (w: pair) (w_left: dict) (w_right: dict) 
    ...
    | Eq -> print_string "******insert_upward_two*******"; raise (DuplicateKeyException (string_of_key w_key))
  ...
  let insert_upward_three (w: pair) (w_left: dict) (w_right: dict)
    ...
    let (y_key, _) = y in
    let (w_key, _) = w in
    match (D.compare w_key y_key) with
    | Greater -> Up(Two(other_left, x, other_right), y, Two(w_left, w, w_right)) 
    | Eq -> print_string "******** insert_upward_three ********"; raise (DuplicateKeyException (string_of_key w_key))
    | Less -> (match (D.compare w_key x_key) with
      | Greater -> Up(Two(other_left, x, w_left), w, Two(w_right, y, other_right)) 
      | Eq -> print_string "******** insert_upward_three ********"; raise (DuplicateKeyException (string_of_key w_key))
  ...
  let rec insert_downward (d: dict) (k: key) (v: value) : kicked =
    match d with
      ...
      | Two(left,(k1,v1),right) -> insert_downward_two (k,v) (k1,v1) left right 
      | Three(left,(k1,v1),middle,(k2,v2),right) ->
	insert_downward_three (k,v) (k1,v1) (k2,v2) left middle right
  ...
  and insert_downward_two ((k,v): pair) ((k1,v1): pair) 
    ...
    | Less -> (
      ...
      | Up(up_left, up_node, up_right) -> insert_upward_two up_node up_left up_right (k1,v1) right
      | Done(new_left) -> Done(Two(new_left,(k1, v1),right))
      )
    | Eq -> Done(Two(left,(k,v),right))
    | Greater -> (
      match insert_downward right k v with
      | Up(up_left, up_node, up_right) -> insert_upward_two up_node up_left up_right (k1,v1) left
  ...
  and insert_downward_three ((k,v): pair) ((k1,v1): pair) ((k2,v2): pair) 
    ...
    | Less -> (
      match insert_downward left k v with
      | Up(up_left, up_node, up_right) -> insert_upward_three up_node up_left up_right (k1,v1) (k2,v2) middle right
	...
	| Up(up_left, up_node, up_right) -> insert_upward_three up_node up_left up_right (k1,v1) (k2,v2) left right 
	| Done(new_middle) -> Done(Three(left, (k1, v1), new_middle, (k2, v2), right))
      )
      | Eq -> Done(Three(left,(k1,v1),middle,(k,v),right))
      | Greater -> (
	match insert_downward right k v with
	| Up(up_left, up_node, up_right) -> insert_upward_three up_node up_left up_right (k1,v1) (k2,v2) left middle
...
(*    print_string ("Inserting (" ^ (string_of_key k) ^ ", " ^ (string_of_value v) ^ ") into " ^ string_of_tree d ^ "\n"); 
*)  match insert_downward d k v with
   ...
   * ANSWER: We recursively determine the length of each path of 
   * each subtree. At each recursive step, if the lengths do not 
   * match, we return -1, which is propagated up through the recursive
   * call stack. Otherwise, if the lengths of the subtrees match, the 
   * length is incremented at the node and returned. If the function
   * terminates with a value other than -1, all paths on the tree have
   * same length.
   *)
  let rec balanced (d: dict) : bool =
    let rec length (d: dict) : int = 
      match d with 
      | Leaf -> 1
      | Two(left, (k1, v1), right) -> (
	let len_left = length left in
	let len_right = length right in
	if (len_left = -1) || (len_right = -1) || (len_left <> len_right) 
	then -1
	else len_left + 1
      )
    | Three(left, (k1,v1), mid, (k2, v2), right) -> (
      let len_left = length left in
      let len_mid = length mid in
      let len_right = length right in 
      if (len_left = -1) || (len_mid = -1) || (len_right = -1) 
	|| (len_left <> len_mid) || (len_mid <> len_right)
      then -1
      else len_left + 1
    )
    in ((length d) <> -1) 
  ...
  let test_choose_empty_set () = 
    assert (choose empty = None);
    ()
      
  let test_choose_nonempty_set () = 
    assert (choose (insert_list empty (generate_pair_list 26)) <> None);
    ()

  let test_lookup_contains () = 
    let k = D.gen_key() in
    let v = D.gen_value() in
    assert (lookup (insert empty k v) k = Some v);
    ()
      
  let test_lookup_not_contains () = 
    let k = D.gen_key() in
    let v = D.gen_value() in 
    let k1 = D.gen_key_gt k () in
  ...
  let run_tests () = 
    ...
    test_insert() ;
    test_choose_empty_set() ;
    test_choose_nonempty_set() ;
    test_lookup_contains() ;
    test_lookup_not_contains() ;
    test_lookup_empty() ;
    ()
