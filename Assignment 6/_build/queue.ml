...
module AmortizedQ : QUEUE = struct
  ...
  let rep_inv (lenf,front,lenb,back) = 
    assert (lenf >= 0);
    assert (lenb >= 0); 
    assert (if lenf = 0 then front = LL.empty else front <> LL.empty);
    assert (if lenb = 0 then back = LL.empty else back <> LL.empty);
    assert (lenb <= lenf)
  ;; 
...
module Performance (Q:QUEUE) : PERF = struct
   ...
   *
   * This test checks the performance of the queue by performing n 
   * inserts and returning the time required to perform this set of
   * operations. 
   *)
  let test1 (n:int) : float =
    let rec generate_list (n: int) : int list = 
      match n with 
      | 0 -> []
      | _ -> n :: generate_list (n-1)
    in
    let input_list = generate_list n in
    let do_test () = 
      let _ = List.fold_left (fun queue item -> Q.ins (item, queue)) 
	Q.emp input_list in 
      ()
    in
    time_fun do_test ()
;;
   ...
   * (linear).
   *
   * This test checks the performance of the queue by performing an 
   * equal number of inserts and removes and returning the time 
   * required to perform this set of operations. 
   *)
  let test2 (n:int) : float = 
    let rec generate_list (n: int) : int list = 
      match n with 
      | 0 -> []
      | _ -> n :: generate_list (n-1)
    in
    let input_list = generate_list (n/2) in
    let do_test () = 
      let half_queue = List.fold_left (fun queue item -> Q.ins (item, queue)) 
	Q.emp input_list in 
      let _ = List.fold_left (fun queue item -> 
	match Q.rem queue with 
	| Some (_, q) -> q 
	| None -> Q.emp) half_queue input_list in
      ()
    in
    time_fun do_test ()
;;
